...
function Game(computer_turn_first)
{
  ...
  var board = [
    ...
    // number of reasons for computer to place a piece in a spot on board. Start off with a value less than 0 to signify spot on board could be taken
  ...
  var findBestSpotForComputerChoice = function()
  {
    ...
    for(way in WAYS_TO_WIN)
    {
      ...
      else if(user_icon_counter == 2 && computer_icon_counter == 0) // put a B in empty slot and block um
      {
      	if(val_of_spot_a == EMPTY_ICON)
      ...
      else // place a zero over possible, but not benificial spot
      {
      	if(val_of_spot_a == EMPTY_ICON)
      		if(computer_reasons[ WAYS_TO_WIN[way][0][0]][ WAYS_TO_WIN[way][0][1] ] != "B")
      		{
    ...
    var highest_valued_spot_value = -1;
		...
		for(var row = 0; row < 3; row++)
		{
			for(var col = 0; col < 3; col++)
			{
				// if can 'B'lock user from winning, then choose that spot.
				if (computer_reasons[row][col] == "B")
					return [row, col]
				else
				{
					// save index that has the highest value
		...
		switch(round)
		{
			...
			case 2 : // computer starts second
				if(board[MIDDLE][MIDDLE] == EMPTY_ICON)
					return([MIDDLE,MIDDLE]) //go for defense stratagy
				else
					return([TOP,LEFT]) //go for defense stratagy
				break

			case 3 : // 2nd turn of 1st player
				// keep offense strategy going
				if(board[BOTTOM][RIGHT] == EMPTY_ICON)
					return([BOTTOM, RIGHT]) // continue offensive strategy normally
				else // you are going to win
					return([TOP, RIGHT])
				break;

			case 4 : // 2nd turn of 2nd player
				       ...
				       )
			  {
				...
				else if ( (board[TOP][LEFT] == USER_ICON && board[MIDDLE][RIGHT] == USER_ICON) ||
					        (board[TOP][LEFT] == USER_ICON && board[MIDDLE][BOTTOM] == USER_ICON)
					      )
		    {
		    	return ([TOP, MIDDLE])
		    }
	...
	// 		// temp put board to console
	// this.printComputerReasons = function(){
	// 	console.log(computer_reasons[0][0] + " | " + computer_reasons[0][1] + " | " + computer_reasons[0][2])
	// 	console.log("----------")
	// 	console.log(computer_reasons[1][0] + " | " + computer_reasons[1][1] + " | " + computer_reasons[1][2])
	// 	console.log("----------")
	// 	console.log(computer_reasons[2][0] + " | " + computer_reasons[2][1] + " | " + computer_reasons[2][2])
	// 	console.log("")
	// }
}
